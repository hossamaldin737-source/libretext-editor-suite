 ...
 * ═══════════════════════════════════════════════════════════════════════════
 * 📄 الملف: image-pipeline.ts
 * 📂 المسار: packages/core/src/engines/image-pipeline.ts
 * 🎯 الهدف الرئيسي: محرك معالجة الصور النقطية والمتجهة
 * 📋 المعايير: Zero-Library Image Processing مع قراءة EXIF يدوية
 * 🧪 الاختبارات: tests/engines/image-pipeline.test.ts
 * 🏷️ المعرف: CORE-ENG-004
...
export async function prepareUploadedImage(
  ...
  if (!ctx) throw new Error('[ImagePipeline] تعذر تهيئة سياق الرسم للـ Canvas');
...
export async function transformImage(
  ...
  if (options.filters) ctx.filter = buildFilterString(options.filters);
...
export async function createThumbnail(source: HTMLImageElement | string, size = 120): Promise<string> {
  ...
  ctx.drawImage(image, (size - fitW) / 2, (size - fitH) / 2, fitW, fitH);
