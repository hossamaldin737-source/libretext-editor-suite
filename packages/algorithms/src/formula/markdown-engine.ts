 ...
 * ═══════════════════════════════════════════════════════════════════════════
 * 📄 الملف: markdown-engine.ts
 * 📂 المسار: packages/algorithms/src/formula/markdown-engine.ts
 * 🎯 الهدف الرئيسي: محرك ماركدون أصيل - تحويل MD↔HTML مع دعم LaTeX والجداول
 * 📋 المعايير: صفر مكتبات خارجية، تحويل ثنائي الاتجاه
 * 🧪 الاختبارات: tests/formula/markdown-engine.test.ts
 * 🏷️ المعرف: ALGO-032
...
export class MarkdownEngine {
  ...
  public static getInstance(): MarkdownEngine {
    if (!MarkdownEngine.instance) MarkdownEngine.instance = new MarkdownEngine();
  ...
  public toHtml(markdown: string): string {
    ...
    text = text.replace(/```([\s\S]*?)```/g, (_m, code) => {
      ...
      return createPlaceholder(`<pre class="bg-slate-100 p-3 rounded-lg border border-slate-200 font-mono text-xs overflow-x-auto text-slate-800 my-2"><code>${escaped}</code></pre>`);
    });
    text = text.replace(/\$\$([\s\S]*?)\$\$/g, (_m, math) => createPlaceholder(latexEngine.renderToHtml(math, true)));
    text = text.replace(/\$([^$\n]+)\$/g, (_m, math) => createPlaceholder(latexEngine.renderToHtml(math, false)));
    text = this.parseMarkdownTables(text);
    text = text.replace(/<div class="overflow-x-auto my-3">[\s\S]*?<\/div>/g, (tableHtml) => createPlaceholder(tableHtml));
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (!line) { result.push(''); }
      else if (line.startsWith('<h') || line.startsWith('<ul') || line.startsWith('<ol') || line.startsWith('<li') || line.startsWith('<blockquote') || line.startsWith('<pre') || line.startsWith('<hr') || line.startsWith('<table') || line.startsWith('<div') || line.startsWith('<img') || line.startsWith('%%MD_PROTECTED_BLOCK_')) { result.push(line); }
      else { result.push(`<p class="my-2 leading-relaxed text-slate-800">${line}</p>`); }
    }
    ...
    for (const child of Array.from(node.childNodes)) {
      if (child.nodeType === Node.TEXT_NODE) { result += child.textContent || ''; }
        ...
        switch (tag) {
          ...
          case 'u': result += `<u>${content}</u>`; break;
          case 'del': case 's': case 'strike': result += `~~${content}~~`; break;
          case 'code': result += el.parentElement?.tagName.toLowerCase() === 'pre' ? content : `\`${content}\``; break;
          case 'pre': result += `\n\n\`\`\`\n${el.textContent || ''}\n\`\`\`\n\n`; break;
          case 'blockquote': result += `\n\n> ${content.replace(/\n/g, '\n> ')}\n\n`; break;
          case 'li': result += `\n- ${content}`; break;
          case 'hr': result += '\n\n---\n\n'; break;
          case 'br': result += '\n'; break;
          case 'a': result += `[${content}](${el.getAttribute('href') || '#'})`; break;
          case 'img': result += `![${el.getAttribute('alt') || 'image'}](${el.getAttribute('src') || ''})`; break;
  ...
  private tableToMarkdown(tableEl: HTMLElement): string {
    ...
    for (const row of rows) {
      ...
      if (isFirstRow) { lines.push('| ' + cells.map(() => '---').join(' | ') + ' |'); isFirstRow = false; }
    }
    return lines.join('\n');
  }
  ...
  private parseMarkdownTables(text: string): string {
    return text.replace(/((?:\|[^\n]+\|\n?)+)/g, (match) => {
      ...
      for (let i = 0; i < lines.length; i++) {
        const line = lines[i]!.trim();
        if (line.includes('---')) { inHeader = false; continue; }
        const cells = line.split('|').filter((_, idx, arr) => idx > 0 && idx < arr.length - 1);
        if (inHeader && i === 0) {
          html += '<thead class="bg-slate-100 text-slate-700 font-semibold border-b border-slate-300"><tr>';
          for (const cell of cells) html += `<th class="border border-slate-300 px-3 py-2 text-right">${cell.trim()}</th>`;
          html += '</tr></thead><tbody>';
        } else {
          html += '<tr class="hover:bg-slate-50 border-b border-slate-200">';
          for (const cell of cells) html += `<td class="border border-slate-300 px-3 py-2 text-right">${cell.trim()}</td>`;
          html += '</tr>';
        }
      }
  ...
  public detectFormat(content: string): 'markdown' | 'html' | 'unknown' {
    ...
    if (trimmed.startsWith('<!DOCTYPE') || trimmed.startsWith('<html') || (trimmed.startsWith('<div') && trimmed.endsWith('</div>'))) return 'html';
    if (/^#{1,6}\s/m.test(trimmed) || /^\s*[-*+]\s/m.test(trimmed) || /```/.test(trimmed) || /\[.+\]\(.+\)/.test(trimmed)) return 'markdown';
    return 'unknown';
  }
}
