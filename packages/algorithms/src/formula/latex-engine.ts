...
export class LaTeXEngine {
  ...
  public renderToHtml(latex: string, isBlock = false): string {
    if (!latex) return '';
    let expr = latex.trim();
    if (expr.startsWith('$$') && expr.endsWith('$$')) { expr = expr.substring(2, expr.length - 2).trim(); isBlock = true; }
    else if (expr.startsWith('$') && expr.endsWith('$')) { expr = expr.substring(1, expr.length - 1).trim(); }
    ...
    expr = expr.replace(/\\(sin|cos|tan|cot|sec|csc|log|ln|exp|max|min|det|deg)/g, '<span class="math-function font-semibold text-slate-700 mx-0.5">$1</span>');
    expr = this.parseMatrices(expr);
    expr = expr.replace(/\\quad/g, '&emsp;').replace(/\\qquad/g, '&emsp;&emsp;').replace(/\\[,;:!]/g, '&nbsp;');
    if (isBlock) {
  ...
  private parseFractions(str: string): string {
    ...
    while (fracRegex.test(result) && iterations < 10) {
      iterations++;
      result = result.replace(fracRegex, (_m, num, den) =>
  ...
  private parseSquareRoots(str: string): string {
    let result = str;
    result = result.replace(/\\sqrt\[([^\]]+)\]\{([^{}]+)\}/g, (_m, n, content) =>
    ...
    result = result.replace(/\\sqrt\{([^{}]+)\}/g, (_m, content) =>
  ...
  private parseLimits(str: string): string {
    return str.replace(/\\(sum|prod|int|iint|lim)_\{([^{}]+)\}\^\{([^{}]+)\}/g, (_m, op, lower, upper) => {
  ...
  private parseSuperSubscripts(str: string): string {
    ...
    result = result.replace(/([a-zA-Z0-9\)\}\]])\^\{([^{}]+)\}_\{([^{}]+)\}/g, '$1<sup class="text-[0.75em]">$2</sup><sub class="text-[0.75em]">$3</sub>');
  ...
  private parseMatrices(str: string): string {
    return str.replace(/\\begin\{(matrix|pmatrix|bmatrix)\}([\s\S]*?)\\end\{\1\}/g, (_m, type, content) => {
      const rows = content.trim().split('\\\\');
      const rowsHtml = rows.map((r: string) => {
        const cells = r.split('&').map((c: string) => `<td class="px-2 py-1 text-center">${this.renderToHtml(c.trim(), false)}</td>`);
        return `<tr>${cells.join('')}</tr>`;
      }).join('');
      let borderClass = '';
      if (type === 'pmatrix') borderClass = 'border-l-2 border-r-2 border-slate-700 rounded-lg px-1';
      else if (type === 'bmatrix') borderClass = 'border-l-2 border-r-2 border-t-2 border-b-2 border-slate-700 px-1';
  ...
  public renderToSvg(latex: string, displayMode = false): string {
    const plainText = latex
      .replace(/\\frac\{([^{}]+)\}\{([^{}]+)\}/g, '($1)/($2)')
      .replace(/\\sqrt\{([^{}]+)\}/g, '√($1)')
      .replace(/\\int/g, '∫').replace(/\\sum/g, '∑')
      .replace(/\\alpha/g, 'α').replace(/\\beta/g, 'β').replace(/\\pi/g, 'π')
      .replace(/\\times/g, '×').replace(/\\pm/g, '±').replace(/[{}]/g, '');
