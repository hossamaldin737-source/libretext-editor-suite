...
### 📁 graph/ — Graph Algorithms (ALGO-020..023)

| #   | المعرف  | الاسم                           | النوع    | الملف                   | الوصف                        | المعلمات                         | المُعاد                            | التبعيات                |
| --- | ------- | ------------------------------- | -------- | ----------------------- | ---------------------------- | -------------------------------- | ---------------------------------- | ----------------------- |
| 1/4 | ALGO-020 | `detectCycle()`                 | Function | `dependency.ts`         | كشف التبعيات الدائرية       | `graph`                          | `string[] \| null`                 | لا شيء                  |
| 2/4 | ALGO-020 | `topologicalSort()`             | Function | `dependency.ts`         | الفرز الجغرافي               | `graph`                          | `string[] \| null`                 | `detectCycle`            |
| 3/4 | ALGO-021 | `routeAStarVisibility()`        | Function | `routing.ts`            | توجيه A* مرئي               | `start, end, obstacles, options` | `Point2D[]`                        | لا شيء                  |
| 4/4 | ALGO-023 | `computeDiagramRoute()`         | Function | `orthogonal-router.ts`  | توجيه متقدم للرسوم البيانية | `source, target, connector, all` | `ComputedRoute`                    | `routeAStarVisibility`  |

### 📁 tree/ + structure/ + sort/ + lookup/ — Data Structures (ALGO-024..029)

| #   | المعرف  | الاسم                           | النوع    | الملف                   | الوصف                        | المعلمات                         | المُعاد                            | التبعيات                |
| --- | ------- | ------------------------------- | -------- | ----------------------- | ---------------------------- | -------------------------------- | ---------------------------------- | ----------------------- |
| 1/3 | ALGO-024 | `LLRBTree`                      | Class    | `llrb.ts`               | شجرة LLRB متوازنة           | لا شيء                          | `LLRBTree`                         | لا شيء                  |
| 2/3 | ALGO-025 | `DisjointSet`                   | Class    | `disjoint-set.ts`       | مجموعة منفصلة Union-Find     | لا شيء                          | `DisjointSet`                      | لا شيء                  |
| 3/3 | ALGO-026 | `bottomUpMergeSort()`           | Function | `mergesort.ts`          | خوارزمية الدمج               | `arr, comparator?`               | `T[]`                              | لا شيء                  |

### 📁 lookup/ + formula/ + streets/ — Lookup & Search (ALGO-027..030)

| #   | المعرف  | الاسم                           | النوع    | الملف                   | الوصف                        | المعلمات                         | المُعاد                            | التبعيات                |
| --- | ------- | ------------------------------- | -------- | ----------------------- | ---------------------------- | -------------------------------- | ---------------------------------- | ----------------------- |
| 1/4 | ALGO-027 | `HLOOKUP()`                     | Function | `hlookup.ts`            | بحث أفقي                     | `value, table, row, approx?`     | `any`                              | لا شيء                  |
| 2/4 | ALGO-028 | `normalizeFunctionName()`       | Function | `arabic-aliases.ts`     | تطبيع أسماء الدوال العربية   | `name`                           | `string`                           | لا شيء                  |
| 3/4 | ALGO-029 | `calculateStringSimilarity()`   | Function | `similarity.ts`         | تشابه أسماء الشوارع          | `s1, s2`                         | `number`                           | لا شيء                  |
| 4/4 | ALGO-030 | `Point2D` / `AABB`              | Type     | `types.ts`              | أنواع مشتركة                 | لا شيء                          | لا شيء                            | لا شيء                  |
